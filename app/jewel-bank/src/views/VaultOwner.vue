...
import { initJewelBank } from '@/common/jewel-bank';
...
import { findVaultPDA } from '@jewelworks/jewel-farm-ts';
...
export default defineComponent({
  ...
  setup() {
    ...
    const startFresh = async () => {
      ...
      gb = await initJewelBank(getConnection(), getWallet()!);
      await fetchVault();
      if (vault.value) {
        await Promise.all([populateWalletNFTs(), populateJewelBankNFTs()]);
      }
    };
    ...
    const depositJewel = async (
      mint: PublicKey,
      creator: PublicKey,
      source: PublicKey
    ) => {
      const { txSig } = await gb.depositJewelWallet(
        new PublicKey(bank.value!),
    ...
    const withdrawJewel = async (mint: PublicKey) => {
      const { txSig } = await gb.withdrawJewelWallet(
        new PublicKey(bank.value!),
        new PublicKey(vault.value!),
        new BN(1),
        mint
    ...
    const populateJewelBankNFTs = async () => {
      // zero out to begin with
      ...
      if (foundGDRs && foundGDRs.length) {
        ...
        const mints = foundGDRs.map((gdr: any) => {
          return { mint: gdr.account.jewelMint };
        });
    ...
    const moveNFTsOnChain = async () => {
      for (const nft of toVaultNFTs.value) {
        ...
        await depositJewel(nft.mint, creator, nft.pubkey!);
      }
      for (const nft of toWalletNFTs.value) {
        await withdrawJewel(nft.mint);
      }
      await populateWalletNFTs();
      await populateJewelBankNFTs();
    };
