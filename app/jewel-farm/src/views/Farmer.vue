...
import FarmerDisplay from '@/components/jewel-farm/FarmerDisplay.vue';
import Vault from '@/components/jewel-bank/Vault.vue';
import { INFT } from '@/common/web3/NFTget';
import { findFarmerPDA, stringifyPKsAndBNs } from '@jewelworks/jewel-farm-ts';
    ...
    const addJewels = async () => {
      await Promise.all(
        selectedNFTs.value.map((nft) => {
          ...
          addSingleJewel(nft.mint, nft.pubkey!, creator);
        })
      );
      console.log(
        `added another ${selectedNFTs.value.length} jewels into staking vault`
      );
    };
