...
import { createFakeWallet } from '@/common/jewel-bank';
import {
  JewelFarmClient,
  FarmConfig,
  VariableRateConfig,
...
export async function initJewelFarm(
  conn: Connection,
  wallet?: SignerWalletAdapter
) {
  const walletToUse = wallet ?? createFakeWallet();
  const farmIdl = await (await fetch('jewel_farm.json')).json();
  const bankIdl = await (await fetch('jewel_bank.json')).json();
  return new JewelFarm(conn, walletToUse as any, farmIdl, bankIdl);
}

export class JewelFarm extends JewelFarmClient {
  constructor(conn: Connection, wallet: any, farmIdl: Idl, bankIdl: Idl) {
    super(conn, wallet, farmIdl, JEWEL_FARM_PROG_ID, bankIdl, JEWEL_BANK_PROG_ID);
  }
  ...
  async flashDepositWallet(
    ...
    console.log('added extra jewel for farmer', this.wallet.publicKey.toBase58());
