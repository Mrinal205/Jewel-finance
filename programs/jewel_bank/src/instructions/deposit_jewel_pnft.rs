...
pub struct DepositJewelPnft<'info> {
    // bank
...
fn assert_whitelisted<'info>(
    ctx: &Context<DepositJewelPnft<'info>>,
    ...
    if bank.whitelisted_creators > 0 {
        ...
        let metadata = assert_decode_metadata(&mint, &ctx.accounts.jewel_metadata)?;
