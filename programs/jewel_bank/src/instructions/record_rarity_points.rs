...
pub fn handler<'a, 'b, 'c, 'info>(
    ...
    for config in rarity_configs.iter() {
        let jewel_mint = next_account_info(remaining_accs)?;
        let jewel_rarity = next_account_info(remaining_accs)?;
        ...
        let mut jewel_rarity_raw = jewel_rarity.data.borrow_mut();
        jewel_rarity_raw[..8].clone_from_slice(&disc.to_bytes()[..8]);
        jewel_rarity_raw[8..10].clone_from_slice(&config.rarity_points.to_le_bytes());
    }
