...
pub fn handler(ctx: Context<WithdrawTokensAuthority>) -> Result<()> {
    ...
    // EXTREMELY IMPORTANT
    // Make sure that vault_ata != jewel_box PDA, otherwise this ix can be used to bypass frozen lock
    let (jewel_box_pda, _) = anchor_lang::prelude::Pubkey::find_program_address(
        &[
            b"jewel_box".as_ref(),
            vault.key().as_ref(),
