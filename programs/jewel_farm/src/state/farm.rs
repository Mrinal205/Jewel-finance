...
pub struct Farm {
    ...
    /// currently staked jewel count
    pub jewels_staked: u64,

    /// currently staked jewel count, where each jewel is multiplied by its rarity score (1 if absent)
...
impl Farm {
    fn assert_valid_max_counts(&self) -> Result<()> {
        ...
        self.assert_not_too_many_jewels()?;
    ...
    fn assert_not_too_many_jewels(&self) -> Result<()> {
        if self.max_counts.max_jewels > 0 {
            require!(
                self.jewels_staked.try_cast()? <= self.max_counts.max_jewels,
                ErrorCode::TooManyJewelsStaked
            )
        }
        Ok(())
    }
    ...
    pub fn begin_staking(
        ...
        self.jewels_staked.try_add_assign(jewels_in_vault)?;
        self.rarity_points_staked
    ...
    pub fn end_staking(&mut self, now_ts: u64, farmer: &mut Account<Farmer>) -> Result<()> {
        match farmer.state {
            ...
            FarmerState::Staked => {
                ...
                // update farmer
                let (jewels_unstaked, rarity_points_unstaked) =
                ...
                self.jewels_staked.try_sub_assign(jewels_unstaked)?;
    ...
    pub fn stake_extra_jewels(
        ...
        // update farmer
        let (_previous_jewels, previous_rarity_points) = farmer.begin_staking(
            self.config.min_staking_period_sec,
            now_ts,
            jewels_in_vault,
            rarity_points_in_vault,
        ...
        // update farm
        self.jewels_staked.try_add_assign(extra_jewels)?;
        self.rarity_points_staked
