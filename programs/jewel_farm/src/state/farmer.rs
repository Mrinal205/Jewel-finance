...
pub struct Farmer {
    ...
    /// vault storing all of the farmer's jewels
    pub vault: Pubkey,

    pub state: FarmerState,

    /// total number of jewels at the time when the vault is locked
    pub jewels_staked: u64,

    /// total number of jewels * rarity of each jewel (1 if un-appraised)
...
impl Farmer {
    pub fn begin_staking(
        ...
        let previous_jewels_staked = self.jewels_staked;
        let previous_rarity_points_staked = self.rarity_points_staked;
        self.jewels_staked = jewels_in_vault;
        ...
        Ok((previous_jewels_staked, previous_rarity_points_staked))
    }
    ...
    pub fn end_staking_begin_cooldown(
        ...
        let jewels_unstaked = self.jewels_staked;
        let rarity_points_unstaked = self.rarity_points_staked;
        self.jewels_staked = 0; //no rewards will accrue during cooldown period
        self.rarity_points_staked = 0;
        self.cooldown_ends_ts = now_ts.try_add(cooldown_period_sec)?;

        // msg!(
        //     "{} jewels now cooling down for {}",
        //     jewels_unstaked,
        //     self.identity
        // );
        Ok((jewels_unstaked, rarity_points_unstaked))
    }
    ...
    pub fn end_cooldown(&mut self, now_ts: u64) -> Result<()> {
        ...
        // zero everything out
        self.jewels_staked = 0;
        self.rarity_points_staked = 0;
        self.min_staking_ends_ts = 0;
        self.cooldown_ends_ts = 0;

        // msg!(
        //     "jewels now unstaked and available for withdrawal for {}",
        //     self.identity
        // );
        Ok(())
    }
