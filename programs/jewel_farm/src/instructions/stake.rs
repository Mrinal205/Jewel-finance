...
pub fn handler(ctx: Context<Stake>) -> Result<()> {
    if ctx.accounts.vault.jewel_count == 0 {
        return Err(error!(ErrorCode::VaultIsEmpty));
    }

    // lock the vault so the user can't withdraw their jewels
    ...
    // begin staking
    farm.begin_staking(now_ts, vault.jewel_count, vault.rarity_points, farmer)?;

    //collect a fee for staking
    ctx.accounts.transfer_fee()?;

    // msg!("{} jewels staked by {}", farmer.jewels_staked, farmer.key());
    Ok(())
}
