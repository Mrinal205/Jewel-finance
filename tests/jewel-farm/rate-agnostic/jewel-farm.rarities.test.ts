...
describe('rarities', () => {
  ...
  it('correctly counts rarity points during deposits/withdrawals', async () => {
    //add rarities for jewel1 mint
    await gf.setJewelRarities(15);
    ...
    assert(vaultAcc.jewelCount.eq(toBN(20)));
    ...
    assert(vaultAcc.jewelCount.eq(toBN(5)));
    ...
    vaultAcc = await gf.fetchVaultAcc(vault);
    assert(vaultAcc.jewelCount.eq(toBN(25)));
    ...
    vaultAcc = await gf.fetchVaultAcc(vault);
    assert(vaultAcc.jewelCount.eq(toBN(0)));
