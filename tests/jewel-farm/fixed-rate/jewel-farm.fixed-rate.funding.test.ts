...
describe('funding (fixed rate)', () => {
  ...
  beforeEach('preps accs', async () => {
    ...
    totalJewels = gf.calcTotalJewels();
  });
  ...
  it('funds -> gets stakers -> cancels', async () => {
    ...
    await gf.verifyFixedReward(30 * gf.jewel1Amount.toNumber());
    ...
    // ----------------- tests
    //funds
    await gf.verifyFunds(30000, 30000 - 30 * gf.jewel1Amount.toNumber(), 0);
    ...
    //token accounts
    await gf.verifyFunderAccContains(30000 - 30 * gf.jewel1Amount.toNumber());
    await gf.verifyPotContains(pot, 30 * gf.jewel1Amount.toNumber());
  });
  ...
  it('funds -> gets stakers -> waits -> cancels', async () => {
    ...
    await gf.verifyFunds(
      30000,
      30000 - 30 * gf.jewel1Amount.toNumber(),
      30 * gf.jewel1Amount.toNumber()
    );
  ...
  it('funds -> gets stakers -> waits -> cancels -> funds again', async () => {
    ...
    //token accounts
    await gf.verifyFunderAccContains(30000 - 30 * gf.jewel1Amount.toNumber());
    await gf.verifyPotContains(pot, 30000 + 30 * gf.jewel1Amount.toNumber());
  });
});
