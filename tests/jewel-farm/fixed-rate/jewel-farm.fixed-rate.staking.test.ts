...
describe('staking (fixed rate)', () => {
  ...
  it('stakes -> accrues -> claims (multi farmer)', async () => {
    ...
    //verify counts
    await gf.verifyStakedJewelsAndFarmers(2);
  ...
  it('stakes -> accrues -> unstakes (twice) -> claims (multi farmer)', async () => {
    ...
    // verify counts
    await gf.verifyStakedJewelsAndFarmers(0, 0, 0);
  ...
  it('voids reserved amount after farmers unstake', async () => {
    let totalReserve = gf.calcTotalJewelRarity().total.mul(toBN(30));
  ...
  it('flash deposits a jewel', async () => {
    //get the jewels back, we'll need them for 2 separate deposits
    await gf.callWithdraw(gf.jewel1Amount, gf.farmer1Identity);

    const initialDeposit = new BN(1); //drop 1 existing jewel, need to lock the vault
    ...
    assert(vaultAcc.jewelCount.eq(initialDeposit));
    assert(
      vaultAcc.rarityPoints.eq(initialDeposit.mul(toBN(gf.jewel1PerJewelRarity)))
    );
    assert.isTrue(vaultAcc.locked);

    let farmAcc = await gf.fetchFarm();
    assert(farmAcc.stakedFarmerCount.eq(new BN(1)));
    assert(farmAcc.jewelsStaked.eq(initialDeposit));
    assert(
      vaultAcc.rarityPoints.eq(initialDeposit.mul(toBN(gf.jewel1PerJewelRarity)))
    );

    let farmerAcc = (await gf.fetchFarmerAcc(farmer)) as any;
    assert(farmerAcc.jewelsStaked.eq(initialDeposit));
    assert(
      farmerAcc.rarityPointsStaked.eq(
        initialDeposit.mul(toBN(gf.jewel1PerJewelRarity))
      )
    );
  ...
  it('flash deposits a jewel (whitelisted mint)', async () => {
    //get the jewels back, we'll need them for 2 separate deposits
    await gf.callWithdraw(gf.jewel1Amount, gf.farmer1Identity);
    ...
    assert(vaultAcc.jewelCount.eq(initialDeposit.add(flashDeposit)));
    assert(
      vaultAcc.rarityPoints.eq(
        initialDeposit.add(flashDeposit).mul(toBN(gf.jewel1PerJewelRarity))
      )
    );
    assert.isTrue(vaultAcc.locked);
  });

  it('flash deposits a jewel (whitelisted creator)', async () => {
    //get the jewels back, we'll need them for 2 separate deposits
    await gf.callWithdraw(gf.jewel1Amount, gf.farmer1Identity);
