...
describe('jewel bank pnft', () => {
  ...
  it('deposits and withdraws pnft (no ruleset)', async () => {
    ...
    //deposit
    const { ixs } = await gb.buildDepositJewelPnft(
      bank.publicKey,
      vault,
      vaultOwner,
      new BN(1),
    ...
    expect(vaultAcc.jewelCount.toNumber()).to.eq(1);

    //withdraw
    const { ixs: withdrawIxs } = await gb.buildWithdrawJewelPnft(
      bank.publicKey,
      vault,
      vaultOwner,
      new BN(1),
    ...
    expect(vaultAcc.jewelCount.toNumber()).to.eq(0);
  });
