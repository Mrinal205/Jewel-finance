...
import { buildAndSendTx } from '../../src/jewel-common/pnft';
...
describe('Withdraw tokens owned by authority', () => {
  ...
  beforeEach('configures accounts', async () => {
    ...
    // Bonk ATA controlled by jewel farm
  ...
  it('withdraws bonk (no jewels)', async () => {
  ...
  it('withdraws bonk (jewels present)', async () => {
    ({ vaultAuth, jewelBox } = await prepDeposit(vaultOwner));
    let jewelBoxAcc = await gb.fetchJewelAcc(jewel.tokenMint, jewelBox);
    ...
    jewelBoxAcc = await gb.fetchJewelAcc(jewel.tokenMint, jewelBox);
    assert(jewelBoxAcc.amount.eq(jewelAmount));
  });

  it('fails to withdraw actual jewels', async () => {
    ({ vaultAuth, jewelBox } = await prepDeposit(vaultOwner));
    let jewelBoxAcc = await gb.fetchJewelAcc(jewel.tokenMint, jewelBox);
    assert(jewelBoxAcc.amount.eq(jewelAmount));
    ...
    const maliciousMint = jewel.tokenMint;
    ...
    const builder = gb.bankProgram.methods
      ...
      .accounts({
        ...
        vaultAta: jewelBox, //<-- try to withdraw from jewelbox
        mint: maliciousMint, //<-- jewel mint
