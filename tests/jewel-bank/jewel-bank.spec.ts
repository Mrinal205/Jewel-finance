...
describe.only('jewel bank', () => {
  const _provider = AnchorProvider.local();
  const gb = new JewelBankClient(_provider.connection, _provider.wallet as any);
  ...
  describe('jewel operations', () => {
    //global state
    let jewelAmount: anchor.BN;
    let jewel: ITokenData;
    let jewelBox: PublicKey;
    let GDR: PublicKey;

    function printJewelBoxState() {
      console.log('amount', jewelAmount.toString());
      console.log('jewel', stringifyPKsAndBNs(jewel as any));
      console.log('jewelBox', jewelBox.toBase58());
    ...
    async function prepJewel(owner?: Keypair) {
      const jewelAmount = new BN(1 + Math.ceil(Math.random() * 100)); //min 2
      const jewelOwner =
    ...
    beforeEach('creates a fresh jewel', async () => {
      //many jewels, different amounts, but same owner (who also owns the vault)
      ({ jewelAmount, jewel } = await prepJewel(vaultOwner));
    });

    it('deposits jewel', async () => {
      let vaultAuth;
      ({ vaultAuth, jewelBox, GDR } = await prepDeposit(vaultOwner));

      const vaultAcc = await gb.fetchVaultAcc(vault);
      assert(vaultAcc.jewelBoxCount.eq(new BN(1)));
      assert(vaultAcc.jewelCount.eq(jewelAmount));
      assert(vaultAcc.rarityPoints.eq(jewelAmount));

      const jewelBoxAcc = await gb.fetchJewelAcc(jewel.tokenMint, jewelBox);
      assert(jewelBoxAcc.amount.eq(jewelAmount));
      assert.equal(jewelBoxAcc.mint.toBase58(), jewel.tokenMint.toBase58());
      assert.equal(jewelBoxAcc.owner.toBase58(), vaultAuth.toBase58());

      const GDRAcc = await gb.fetchGDRAcc(GDR);
      assert.equal(GDRAcc.vault.toBase58(), vault.toBase58());
      assert.equal(GDRAcc.jewelBoxAddress.toBase58(), jewelBox.toBase58());
      assert.equal(GDRAcc.jewelMint.toBase58(), jewel.tokenMint.toBase58());
      assert(GDRAcc.jewelCount.eq(jewelAmount));
    });

    it('FAILS to deposit jewel w/ wrong owner', async () => {
      await expect(prepDeposit(randomWallet)).to.be.rejectedWith(
        'ConstraintHasOne'
      );
    });

    it('withdraws jewel to existing ATA', async () => {
      ({ jewelBox, GDR } = await prepDeposit(vaultOwner)); //make a fresh deposit

      const vaultAcc = await gb.fetchVaultAcc(vault);
      const oldBoxCount = vaultAcc.jewelBoxCount;
      const oldJewelCount = vaultAcc.jewelCount;

      await prepWithdrawal(vaultOwner, jewel.owner, jewelAmount);

      const vaultAcc2 = await gb.fetchVaultAcc(vault);
      assert(vaultAcc2.jewelBoxCount.eq(oldBoxCount.sub(new BN(1))));
      assert(vaultAcc2.jewelCount.eq(oldJewelCount.sub(jewelAmount)));
      assert(vaultAcc2.rarityPoints.eq(oldJewelCount.sub(jewelAmount)));

      const jewelAcc = await gb.fetchJewelAcc(jewel.tokenMint, jewel.tokenAcc);
      assert(jewelAcc.amount.eq(jewelAmount));

      //these accounts are expected to close on emptying the jewel box
      await expect(gb.fetchJewelAcc(jewel.tokenMint, jewelBox)).to.be.rejectedWith(
    ...
    it('withdraws jewel to existing ATA (but does not empty)', async () => {
      const smallerAmount = jewelAmount.sub(new BN(1));

      ({ jewelBox, GDR } = await prepDeposit(vaultOwner)); //make a fresh deposit

      await prepWithdrawal(vaultOwner, jewel.owner, smallerAmount);

      const jewelAcc = await gb.fetchJewelAcc(jewel.tokenMint, jewel.tokenAcc);
      assert(jewelAcc.amount.eq(smallerAmount));

      const jewelBoxAcc = await gb.fetchJewelAcc(jewel.tokenMint, jewelBox);
      assert(jewelBoxAcc.amount.eq(new BN(1)));

      const GDRAcc = await gb.fetchGDRAcc(GDR);
      assert(GDRAcc.jewelCount.eq(new BN(1)));
    });

    it('withdraws jewel to missing ATA', async () => {
      ({ jewelBox, GDR } = await prepDeposit(vaultOwner)); //make a fresh deposit
      ...
      await prepWithdrawal(vaultOwner, randomWallet.publicKey, jewelAmount);

      const jewelAcc = await gb.fetchJewelAcc(jewel.tokenMint, missingATA);
      assert(jewelAcc.amount.eq(jewelAmount));
    ...
    it('FAILS to withdraw jewel w/ wrong owner', async () => {
      await prepDeposit(vaultOwner); //make a fresh deposit

      await expect(
        prepWithdrawal(randomWallet, jewel.owner, jewelAmount)
      ).to.be.rejectedWith('ConstraintHasOne');
    });
    ...
    it('un/locks vault successfully', async () => {
      ...
      await expect(
        prepWithdrawal(vaultOwner, jewel.owner, jewelAmount)
    ...
    describe('whitelists', () => {
      ...
      it('tries to whitelist same mint twice', async () => {
        await whitelistMint(jewel.tokenMint);
        const { whitelistedMint } = await whitelistMint(jewel.tokenMint);
      ...
      it('allows a deposit if creator verified + whitelisted, and mint WL empty', async () => {
        const jewelMetadata = await createMetadata(
          nw.conn,
          nw.wallet,
          jewel.tokenMint
        );
      ...
      it('allows a deposit if creator verified + whitelisted, and mint WL NOT empty', async () => {
        ...
        const { jewel: randomJewel } = await prepJewel();
        const { whitelistedMint } = await whitelistMint(randomJewel.tokenMint); //random mint intentionally
        const { whitelistedCreator, whitelistProof } = await whitelistCreator(
          nw.wallet.publicKey //this is the address used to create the metadata
        );
      ...
      it('FAILS a deposit if mint whitelist exists, but mint not whitelisted', async () => {
        //setup the whitelist for the WRONG jewel
        const { jewel: randomJewel } = await prepJewel();
        const { whitelistedMint, whitelistProof } = await whitelistMint(
          randomJewel.tokenMint
        );
