...
import { AccountUtils, isKp } from '../jewel-common';
import { JewelBank } from '../types/jewel_bank';
import {
  findGdrPDA,
  findJewelBoxPDA,
  findRarityPDA,
...
export class JewelBankClient extends AccountUtils {
  wallet: anchor.Wallet;
  provider!: anchor.Provider;
  bankProgram!: anchor.Program<JewelBank>;
  ...
  setBankProgram(idl?: Idl, programId?: PublicKey) {
    ...
    if (idl && programId) {
      ...
      this.bankProgram = new anchor.Program<JewelBank>(
        idl as any,
        programId,
        this.provider
      );
      ...
      this.bankProgram = anchor.workspace.JewelBank as Program<JewelBank>;
    }
  }
  ...
  async fetchGDRAcc(GDR: PublicKey) {
    return this.bankProgram.account.jewelDepositReceipt.fetch(GDR);
  }

  async fetchJewelAcc(mint: PublicKey, jewelAcc: PublicKey) {
    return this.deserializeTokenAccount(mint, jewelAcc);
  }
  ...
  async fetchAllGdrPDAs(vault?: PublicKey) {
    const filter = vault
      ...
      : [];
    const pdas = await this.bankProgram.account.jewelDepositReceipt.all(filter);
  ...
  async depositJewel(
    ...
    if (pnft) {
      const {
        ...
        jewelRarityBump,
        ixs,
      } = await this.buildDepositJewelPnft(
        bank,
        vault,
        vaultOwner,
        jewelAmount,
  ...
  async buildDepositJewel(
    ...
    const [jewelBox, jewelBoxBump] = await findJewelBoxPDA(vault, jewelMint);
    const [GDR, GDRBump] = await findGdrPDA(vault, jewelMint);
    const [vaultAuth, vaultAuthBump] = await findVaultAuthorityPDA(vault);
    const [jewelRarity, jewelRarityBump] = await findRarityPDA(bank, jewelMint);
    ...
    console.log(
      `depositing ${jewelAmount} jewels into ${jewelBox.toBase58()}, GDR ${GDR.toBase58()}`
    );
    const builder = this.bankProgram.methods
      .depositJewel(vaultAuthBump, jewelRarityBump, jewelAmount)
      .accounts({
        bank,
        vault,
  ...
  async buildDepositJewelPnft(
    ...
    console.log(
      `depositing ${jewelAmount} jewels into ${jewelBox.toBase58()}, GDR ${GDR.toBase58()} (PNFT)`
    );
    const builder = this.bankProgram.methods
      .depositJewelPnft(
        vaultAuthBump,
  ...
  async withdrawJewel(
    ...
    if (pnft) {
      const {
        ...
        jewelDestination,
        ixs,
      } = await this.buildWithdrawJewelPnft(
        bank,
        vault,
        vaultOwner,
        jewelAmount,
  ...
  async buildWithdrawJewel(
    ...
    const jewelDestination = await this.findATA(jewelMint, receiver);

    const signers = [];
    if (isKp(vaultOwner)) signers.push(<Keypair>vaultOwner);

    console.log(
      `withdrawing ${jewelAmount} jewels from ${jewelBox.toBase58()}, GDR ${GDR.toBase58()}`
    );
    const builder = this.bankProgram.methods
      .withdrawJewel(vaultAuthBump, jewelBoxBump, GDRBump, jewelRarityBump, jewelAmount)
      .accounts({
        bank,
        vault,
        owner: isKp(vaultOwner) ? (<Keypair>vaultOwner).publicKey : vaultOwner,
  ...
  async buildWithdrawJewelPnft(
    ...
    console.log(
      `withdrawing ${jewelAmount} jewels from ${jewelBox.toBase58()}, GDR ${GDR.toBase58()} (PNFT)`
    );
    const builder = this.bankProgram.methods
      .withdrawJewelPnft(
        vaultAuthBump,
