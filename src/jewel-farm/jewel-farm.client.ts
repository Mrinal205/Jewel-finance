...
import { JewelFarm } from '../types/jewel_farm';
import { isKp } from '../jewel-common';
import {
  findGdrPDA,
  findJewelBoxPDA,
  findRarityPDA,
...
export class JewelFarmClient extends JewelBankClient {
  farmProgram!: anchor.Program<JewelFarm>;
  ...
  setFarmProgram(idl?: Idl, programId?: PublicKey) {
    ...
    if (idl && programId) {
      ...
      this.farmProgram = new anchor.Program<JewelFarm>(
        idl as any,
        programId,
        this.provider
      );
      ...
      this.farmProgram = anchor.workspace.JewelFarm as Program<JewelFarm>;
    }
  }
  ...
  async initFarm(
    ...
    const txSig = await this.farmProgram.methods
      ...
      .accounts({
        ...
        rewardBMint,
        bank: bank.publicKey,
        jewelBank: this.bankProgram.programId,
  ...
  async flashDeposit(
    ...
    const [jewelRarity, jewelRarityBump] = await findRarityPDA(
      farmAcc.bank,
      jewelMint
    );
  ...
  async addRaritiesToBank(
    ...
    for (const config of completeRarityConfigs) {
      const [jewelRarity] = await findRarityPDA(bank, config.mint);
      //add mint
      remainingAccounts.push({
        pubkey: config.mint,
        isWritable: false,
